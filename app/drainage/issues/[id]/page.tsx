...
export default function DrainageIssueDetailPage() {
  ...
  const load = () => {
    const found = MockDrainageData.getIssue(params.id);
    ...
    return (
      <div className="min-h-screen bg-[#E7ECF0]">
        ...
        <div className="flex justify-center items-center py-20">
          <div className="font-mono text-sm text-slate-600">Loading Drainage issue details...</div>
        </div>
      </div>
    );
  }
  ...
  function confirm() {
    ...
    try {
      const all = MockDrainageData.getIssues();
      ...
      if (idx !== -1) {
        ...
        localStorage.setItem("civic_Drainage_issues", JSON.stringify(all));
      }
  ...
  const damageClassLabel = DRAINAGE_CLASSES.find(c => c.id === issue.damageClassId)?.label || issue.damageClassId.replace(/_/g, " ");
