...
export default function SanitationReportPage() {
  ...
  async function submit() {
    ...
    setTimeout(() => {
      try {
        const randomClass = SANITATION_CLASSES[Math.floor(Math.random() * SANITATION_CLASSES.length)];
        
        const newIssue = MockSanitationData.createIssue({
          reporterId: user.id,
          lat: coords.lat,
          lng: coords.lng,
          imageUrl: preview || "",
        ...
        <div className="mb-6 bg-white/80 backdrop-blur-sm p-6 rounded-2xl border border-[#CBD5E1] shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-600 font-medium">Capture or upload photo · AI verifies and routes to Sanitation immediately</p>
            </div>
          </div>
        </div>
              ...
              <div className="rounded-2xl border-2 border-dashed border-[#CBD5E1] bg-white p-8 text-center shadow-sm">
                {preview ? (
                  ...
                  <div className="flex flex-col items-center gap-3 text-slate-500 py-6">
                    ...
                    <p className="text-xs text-slate-500 max-w-xs">Upload a clear photo of the sanitation issue (garbage accumulation, illegal dumping, blocked drains)</p>
                  </div>
                )}
                <input
                  ref={fileRef}
              ...
              {result.kind === "rejected" ? (
                ...
                <div className="rounded-2xl border border-emerald-200 bg-white p-6 shadow-sm">
                  ...
                  <div className="mt-5 flex gap-3">
                    <button onClick={() => router.push(`/sanitation/issues/${result.issueId}`)} className="rounded-xl bg-[#FFC000] hover:bg-[#EBB000] px-5 py-2.5 text-xs font-bold uppercase tracking-wider text-slate-950 shadow-sm">
                      View Issue Status
                    </button>
                    <button onClick={() => router.push("/sanitation/dashboard")} className="rounded-xl bg-slate-100 hover:bg-slate-200 px-5 py-2.5 text-xs font-bold uppercase tracking-wider text-slate-900">
