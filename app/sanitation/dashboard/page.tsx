...
export default function SanitationDashboardPage() {
  ...
  const [damageClasses, setDamageClasses] = useState<DamageClass[]>(SANITATION_CLASSES);
  ...
  useEffect(() => {
    ...
    const myIssues = MockSanitationData.getIssues().filter((i) => i.reporterId === user.id);
  ...
  // Calculate local mock impact for sanitation
  const mockImpact = {
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 py-10">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4 bg-white/80 backdrop-blur-sm p-6 rounded-2xl border border-[#CBD5E1] shadow-sm">
          <div>
            ...
            <p className="text-xs text-slate-600 font-medium">Your personal civic impact & sanitation verification stats.</p>
          </div>
          <Link
          ...
          >
            <Camera size={16} /> Report Sanitation Issue
          </Link>
        </div>
        ...
        {data && (
          <>
            ...
            <div className="grid grid-cols-3 gap-4 text-center mb-8">
              <StatCard label="Sanitation Issues Reported" value={mockImpact.reported} />
        ...
        <div>
          <h2 className="mb-3 font-display text-xl font-black uppercase tracking-tight text-slate-950">My Submitted Sanitation Reports</h2>
          <div className="space-y-3">
            {issues.length === 0 && <p className="text-xs text-slate-500 bg-white p-5 rounded-xl border border-slate-200">You haven't reported any sanitation issues yet.</p>}
            {issues.map((i) => (
              <IssueCard key={i.id} issue={i} damageClass={damageClasses.find((d) => d.id === i.damageClassId)} href={`/sanitation/issues/${i.id}`} />
            ))}
          </div>
        </div>
      </div>
